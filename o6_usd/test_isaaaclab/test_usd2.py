"""
test:
    1. isaac sim 能够正常导入 usd (DONE)
    2. joint: 6个主动关节+5个从动关节 运行正常
        print 关节名称 & 活动上下限 ; 并检查控制指令和sim效果是否对应 (DONE)
        联动设置 (TODO)
    3. 碰撞正常 (TODO)
    4. 其他：
        基础（灯光，桌面，视角）; (DONE)
...
HAND_CONFIG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path="./o6_sim/o6_urdf/linkerhand_o6_right/linkerhand_o6_right.usd",
...
def main():
    ...
    # 握拳/弯曲
    pose_close = {
        "rh_thumb_cmc_yaw": 0.2,     
        "rh_thumb_cmc_pitch": 0.50,   
        "rh_index_mcp_pitch": 0.55,   
        "rh_middle_mcp_pitch": 1.55,  
        "rh_ring_mcp_pitch": 1.55,    
        "rh_pinky_mcp_pitch": 1.55    
    }

    # 串联动作序列：先弯曲 -> 再打开 -> 再弯曲
    keyframes = [pose_close]  #[pose_close, pose_open, pose_close, pose_open, pose_close]
    ...
    # 根据 URDF 定义联动规则库
    # 格式： "从动关节名": ("主动关节名", multiplier, offset)
    mimic_rules = {
    ...
    for pose in keyframes:
        ...
        # 2. 强制执行联动规则 (计算从动关节)
        for mimic_joint, (master_joint, mult, offset) in mimic_rules.items():
            if mimic_joint in name_to_index and master_joint in name_to_index:
                master_val = q[0, name_to_index[master_joint]]
                # 联动公式计算
                q[0, name_to_index[mimic_joint]] = master_val * mult + offset 
